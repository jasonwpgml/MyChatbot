...
- **모델 선택 기능(도전 과제 4)**: 사이드바에서 GPT 모델을 선택할 수 있다. 어떤 모델이 답했는지 추적하기 위해 `message` 테이블에 `model` 컬럼을 추가했고, 답변·오류 메시지 아래에 "모델명 · 시각" 형식으로 표시된다. role 컬럼에 모델명을 섞지 않고 별도 컬럼으로 둔 이유는, role은 user/assistant 구분과 OpenAI API 문맥 전달에 그대로 사용되기 때문이다. 기존 DB는 `init_db()`에서 `ALTER TABLE`로 자동 마이그레이션된다.
